...
function getDatabasePath(workspaceFolder) {
    if (!workspaceFolder) {
        return null;
    }
...
async function activate(context) {
    ...
    try {
        ...
        const viewResultsCommand = vscode.commands.registerCommand('recon-again.viewResults', async (item, ...args) => {
        ...
        const showVisualizationCommand = vscode.commands.registerCommand('recon-again.showVisualization', () => {
            ...
            visualization_1.VisualizationPanel.createOrShow(extensionPath, context);
        });
    ...
    if (workspaceFolder) {
        ...
        if (dbPath) {
            const watcher = vscode.workspace.createFileSystemWatcher(new vscode.RelativePattern(workspaceFolder, 'data/recon_again.db'));
            watcher.onDidChange(() => {
                if (resultsProvider) {
                    resultsProvider.refresh();
                }
...
async function runSpecificTool() {
    ...
    try {
        ...
        const tools = await vscode.window.showQuickPick(['crt_sh', 'urlscan', 'hibp', 'sublist3r', 'dnsrecon', 'wayback', 'sherlock'], {
        ...
        try {
            if (!element) {
                ...
                const sessionItems = sessions.map(s => {
                    ...
                    return new SessionItem(`${s.target} (${s.status})`, vscode.TreeItemCollapsibleState.Collapsed, s.session_id, s.target, s.status, startTime);
                });
                db.close();
                ...
                for (const tr of toolResults) {
                    items.push(new ToolResultItem(tr.tool_name, vscode.TreeItemCollapsibleState.None, element.sessionId, tr.tool_name, tr.success === 1));
                }
                // Add AI analysis if available
                if (aiAnalysisRows.length > 0) {
                    items.push(new ToolResultItem('AI Analysis', vscode.TreeItemCollapsibleState.None, element.sessionId, 'ai_analysis', true));
                }
                db.close();
...
class SessionItem extends vscode.TreeItem {
    constructor(label, collapsibleState, sessionId, target, status, startTime) {
...
class ToolResultItem extends vscode.TreeItem {
    constructor(label, collapsibleState, sessionId, toolName, success) {
        super(label, collapsibleState);
        this.label = label;
        this.collapsibleState = collapsibleState;
        this.sessionId = sessionId;
