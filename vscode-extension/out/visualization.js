...
class VisualizationPanel {
    static createOrShow(extensionPath, context) {
        ...
        const panel = vscode.window.createWebviewPanel(VisualizationPanel.viewType, 'Recon-Again Visualization', column || vscode.ViewColumn.One, {
            enableScripts: true,
            localResourceRoots: [
    ...
    constructor(panel, extensionPath, context) {
        this._disposables = [];
        this._panel = panel;
...
<body>
        ...
        const sessionsHtml = data.recentSessions.map((session) => `
            <div class="session-item" onclick="viewSession('${session.session_id}')">
                ...
                <div class="session-tools">
                    ${(session.tools || []).map((tool) => `<span class="tool-badge">${tool}</span>`).join('')}
                </div>
            </div>
        `).join('');
        return `
    ...
    _renderChartScripts(data) {
        const toolNames = Object.keys(data.toolStats || {});
        const toolCounts = toolNames.map((name) => (data.toolStats[name]?.count || 0));
        const toolSuccess = toolNames.map((name) => (data.toolStats[name]?.success || 0));
        const toolTimes = toolNames.map((name) => (data.toolStats[name]?.avgTime || 0));
        return `
...
VisualizationPanel.viewType = 'reconAgainVisualization';
